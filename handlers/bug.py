...
@whitelist_only
async def start_bug(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Start the bug report conversation."""
    context.user_data["issue_type"] = "bug"
    await update.message.reply_text(
        "Let's report a bug!\n\n"
        "What's the title/summary of the bug?\n"
        "(Send /cancel to stop)"
    )
    return TITLE
...
async def receive_description(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        [InlineKeyboardButton("Skip screenshot", callback_data="bug_skip_screenshot")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)

    await update.message.reply_text(
        "Got it!\n\n"
        "Now send a screenshot of the bug (highly recommended).\n"
...
async def show_impact_selection(
    ...
    keyboard = [
        [
            InlineKeyboardButton("High", callback_data="bug_impact_high"),
            InlineKeyboardButton("Mid", callback_data="bug_impact_mid"),
            InlineKeyboardButton("Low", callback_data="bug_impact_low"),
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)

    message = "What's the business impact of this bug?"
...
async def receive_impact(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    impact = query.data.replace("bug_impact_", "")
    context.user_data["impact"] = impact

    keyboard = [
        [
            InlineKeyboardButton("ASAP", callback_data="bug_urgency_asap"),
            InlineKeyboardButton("Soon", callback_data="bug_urgency_soon"),
            InlineKeyboardButton("Future", callback_data="bug_urgency_future"),
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def receive_urgency(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    urgency = query.data.replace("bug_urgency_", "")
    context.user_data["urgency"] = urgency

    await query.edit_message_text("Creating bug report on GitHub...")
    ...
    try:
        ...
        await query.edit_message_text(
            f"Failed to create bug report: {str(e)}\n\n"
...
bug_handler = ConversationHandler(
    entry_points=[CommandHandler("bug", start_bug)],
    states={
        TITLE: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_title)],
        DESCRIPTION: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_description)],
        SCREENSHOT: [
            MessageHandler(filters.PHOTO, receive_screenshot),
            CallbackQueryHandler(skip_screenshot, pattern="^bug_skip_screenshot$"),
        ],
        IMPACT: [CallbackQueryHandler(receive_impact, pattern="^bug_impact_")],
        URGENCY: [CallbackQueryHandler(receive_urgency, pattern="^bug_urgency_")],
    },
